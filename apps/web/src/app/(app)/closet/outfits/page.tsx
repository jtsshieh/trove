...
async function OutfitsGrid() {
	...
	// Fire the outfits prefetch WITHOUT awaiting — the client dehydrates the pending
	// query so OutfitBuilder's useSuspenseQuery streams (matching the other apps) and
	// resolves on the client where the QueryClientProvider lives.
	void queryClient.prefetchQuery({
		queryKey: outfitsQueryOptions.queryKey,
		queryFn: getAllOutfits,
	});
	const [wardrobe, trips] = await Promise.all([getAllClothes(), getAllTrips()]);
