...
type Suggestion = Record<string, unknown> | null | undefined;

interface GearItem<D> {
	id: string;
...
export interface GearBulkAddConfig<D> {
	/** Lowercase singular + plural, e.g. "container" / "containers". */
	noun: string;
	nounPlural: string;
	triggerTestId: string;
	dialogTestId: string;
	newDraft: () => D;
	/** True when the row has the minimum fields to be created. */
	isReady: (draft: D) => boolean;
	/** Create one row; throws on failure. */
	createItem: (draft: D, imageKey: string | undefined) => Promise<unknown>;
	/** Scan one uploaded image for suggested fields. */
	scanItem: (imageKey: string) => Promise<{ suggestion: Suggestion }>;
	/** Merge a fresh scan suggestion into a draft (keep any quantity already set). */
	applySuggestion: (draft: D, suggestion: Suggestion) => D;
	/** Query key to invalidate after a successful batch. */
	invalidateKey: readonly unknown[];
	/** The per-row field inputs (name / type / quantity / ...). */
	renderFields: (args: {
		draft: D;
		patch: (patch: Partial<D>) => void;
		disabled: boolean;
		idBase: string;
	}) => ReactNode;
}
...
function nextId() {
	counter += 1;
	return `gear-bulk-${counter}`;
}

export function GearBulkAddDialog<D>(config: GearBulkAddConfig<D>) {
	const queryClient = useQueryClient();
	const [open, setOpen] = useState(false);
	const [items, setItems] = useState<GearItem<D>[]>([]);
	...
	const itemsRef = useRef<GearItem<D>[]>(items);
	...
	function patch(id: string, fn: (item: GearItem<D>) => GearItem<D>) {
	...
	async function scanOne(id: string) {
		...
		const keyIfCurrent = (i: GearItem<D>) =>
			i.file === fileAtScan ? imageKey : i.imageKey;
		try {
			const res = await config.scanItem(imageKey);
			patch(id, (i) => ({
				...i,
				scan: 'done',
				scanError: undefined,
				imageKey: keyIfCurrent(i),
				draft: config.applySuggestion(i.draft, res.suggestion),
			}));
		} catch {
			patch(id, (i) => ({
				...i,
	...
	// Replace a photo with its edited version. Same item, so the scan draft + bg
	// state stay valid; only the uploaded copy is stale, so null the key.
	...
	function patchDraft(id: string, draftPatch: Partial<D>) {
		patch(id, (i) => ({ ...i, draft: { ...i.draft, ...draftPatch } }));
	}

	async function createAll() {
		const ready = items.filter((i) => !i.created && config.isReady(i.draft));
		if (ready.length === 0) {
			toast.error('Fill in the details on at least one item');
			return;
		}
		setCreating(true);
		...
		const results = await Promise.all(
			ready.map(async (i) => {
				try {
					...
					await config.createItem(i.draft, imageKey);
		...
		if (failed.size === 0) {
			toast.success(
				`Added ${createdCount} ${createdCount === 1 ? config.noun : config.nounPlural}`,
			);
			setOpen(false);
			reset();
		} else {
	...
	const readyCount = items.filter(
		(i) => !i.created && config.isReady(i.draft),
	).length;
		...
		>
			<DialogTrigger
				render={
					<Button
						...
						data-testid={config.triggerTestId}
					/>
				}
			>
				<Layers />
			...
			>
				<DialogHeader className="border-b p-4">
					<DialogTitle>Bulk add {config.nounPlural}</DialogTitle>
					<DialogDescription>
						{stage === 'select'
...
function ReviewStage<D>({
	...
	onPatch: (id: string, patch: Partial<D>) => void;
	onRemove: (id: string) => void;
	onEdit: (id: string) => void;
	renderFields: GearBulkAddConfig<D>['renderFields'];
}) {
	return (
	...
	return (
		...
		>
			...
			<div className="flex min-w-0 flex-1 flex-col gap-2">
				<div
					className={cn(
						'grid gap-2 transition-opacity sm:grid-cols-2',
