...
import { createEssentialBatch, scanEssentialImage } from './_data/api';
import { essentialsKeys } from './_data/queries';

const CATEGORIES = Object.keys(EssentialCategory);
...
function applySuggestion(
	...
	if (suggestion) {
		...
		if (
			...
			d.category = suggestion.category;
		}
	}
	return d;
}
...
function nextId() {
	counter += 1;
	return `bulk-ess-${counter}`;
}

export function EssentialBulkAddDialog() {
	...
	function isReady(i: BulkItem) {
		return !i.created && i.draft.name.trim() && i.draft.category;
	}

	async function createAll() {
		const ready = items.filter(isReady);
		if (ready.length === 0) {
			toast.error('Fill name and category on at least one item');
			return;
		}
		setCreating(true);
		try {
			...
			const data = await createEssentialBatch(
				prepared.map(({ item: i, imageKey }) => ({
					name: i.draft.name.trim(),
					category: i.draft.category as EssentialCategory,
	...
	return (
		...
		>
			<DialogTrigger
				render={
					<Button
						...
						data-testid="essential-bulk-add-trigger"
					/>
				}
			>
				<Layers />
			...
			<DialogContent
				...
				data-testid="essential-bulk-add-dialog"
			>
				<DialogHeader className="border-b p-4">
					<DialogTitle>Bulk add essentials</DialogTitle>
					<DialogDescription>
						{stage === 'select'
				...
				>
					...
					<div className="flex flex-col gap-1">
						...
						<Select
							...
							onValueChange={(v) => onPatch(item.id, { category: v ?? '' })}
							disabled={fieldsDisabled}
						>
							<SelectTrigger id={`${id}-category`} size="sm" data-testid="bulk-category">
