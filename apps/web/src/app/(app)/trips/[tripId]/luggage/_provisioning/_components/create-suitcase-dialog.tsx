...
export function CreateSuitcaseDialog({
	...
	// Read the watched value once at the top level (watch() is a hook — never call
	// it inside a loop/predicate like `.find()`), then match against it.
	const luggageId = form.watch('luggageId');
	const selected = luggage.find((l) => l.id === luggageId);
