...
import type { Brand, ClothingType } from '@/generated/prisma/client';
import {
	AlertCircle,
	Check,
	CheckCircle2,
...
import { createClothingBatch, scanClothingImage } from './_data/api';
...
function emptyDraft(): Draft {
	return { type: '', brand: '', color: '', brandLine: '', quantity: 1 };
}

/** Overwrite the scannable fields from a fresh suggestion; keep the quantity the
 *  user may have already set. */
function applySuggestion(
	base: Draft,
	suggestion: Record<string, unknown> | null | undefined,
): Draft {
	const d: Draft = { ...emptyDraft(), quantity: base.quantity };
	if (suggestion) {
		if (typeof suggestion.type === 'string') d.type = suggestion.type;
		if (
			suggestion.typeCategory === 'Top' ||
			suggestion.typeCategory === 'Bottom' ||
			suggestion.typeCategory === 'Accessory'
		) {
			d.typeCategory = suggestion.typeCategory;
		}
		if (typeof suggestion.brand === 'string') d.brand = suggestion.brand;
		if (typeof suggestion.color === 'string') d.color = suggestion.color;
		if (typeof suggestion.brandLine === 'string') d.brandLine = suggestion.brandLine;
	}
	return d;
}
...
/** Lets a freshly scanned (not-in-catalog) value still render as a Select option. */
function withCurrent(known: string[], current: string) {
...
export function BulkAddDialog({
	...
	async function scanOne(id: string) {
		...
		try {
			...
			patch(id, (i) => ({
				...
				draft: applySuggestion(i.draft, suggestion),
			}));
		} catch {
			patch(id, (i) => ({
				...i,
	...
	// same garment, so the scan draft + bg state stay valid; only the uploaded copy
	...
	async function createAll() {
		...
		if (ready.length === 0) {
			toast.error('Fill type, brand, and color on at least one item');
			return;
		}
		setCreating(true);
		try {
			...
			const data = await createClothingBatch(
				prepared.map(({ item: i, imageKey }) => ({
					type: i.draft.type.trim(),
					typeCategory: i.draft.typeCategory,
					brand: i.draft.brand.trim(),
					color: i.draft.color.trim(),
					brandLine: i.draft.brandLine.trim() || undefined,
			...
			await queryClient.invalidateQueries({ queryKey: wardrobeKeys.clothing });
			await queryClient.invalidateQueries({
				queryKey: wardrobeKeys.clothingTypes,
			});
			if (failedIdx.size === 0) {
				toast.success(
					`Added ${data.created} item${data.created === 1 ? '' : 's'}`,
				);
				setOpen(false);
				reset();
			} else {
		...
		>
			...
			>
				<DialogHeader className="border-b p-4">
					<DialogTitle>Bulk add clothing</DialogTitle>
					<DialogDescription>
						{stage === 'select'
			...
			<div className="flex min-w-0 flex-1 flex-col gap-2">
				...
				>
					<DraftSelect
						...
						placeholder="Type"
						value={draft.type}
						options={withCurrent(typeNames, draft.type)}
					...
					<DraftSelect
						...
						placeholder="Brand"
						value={draft.brand}
						options={withCurrent(brandNames, draft.brand)}
						disabled={fieldsDisabled}
						testid="bulk-brand"
					...
					<DraftSelect
						...
						placeholder="Color"
						value={draft.color}
						options={withCurrent(colors, draft.color)}
						disabled={fieldsDisabled}
						testid="bulk-color"
						onChange={(value) => onPatch(item.id, { color: value })}
					/>
					<div className="flex flex-col gap-1">
						<Label htmlFor={`${id}-brandline`} className="text-xs">
							Brand line
						</Label>
						<Input
							id={`${id}-brandline`}
							...
							data-testid="bulk-brandline"
							onChange={(e) => onPatch(item.id, { brandLine: e.target.value })}
						/>
					</div>
					<div className="flex flex-col gap-1">
	...
	return (
		<div className="flex flex-col gap-1">
			...
			<Select
				...
				disabled={disabled}
			>
				<SelectTrigger id={id} size="sm" data-testid={testid}>
